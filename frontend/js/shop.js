...
function makeShopCell(birdColor = 'Red', price = 200) {
    ...
    cell.innerHTML += `
        <img src="./frontend/images/Birds/${birdColor}.png" alt="${birdColor}" />
		<br>
        <h3>${name}</h3>
        <div id="buy-${birdColor}">
            <button id="buy${birdColor}" class="buy-button"
                data-color="${birdColor}" data-price=${price}>Kupi ovog pokemona</button>
            <button id="equip${birdColor}" class="equip-button"
                data-color="${birdColor}" style="display:${includesColor ? 'block' : 'none'};">Ne koristi</button>
        </div>
...
function setPlayerCoins() {
    ...
    coinsContainer.innerHTML = `Imas ${getCookieData('coinAmount') || 0}<div id="coinCounterImage"></div>  maraka`;
}
